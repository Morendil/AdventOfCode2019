...
load program = do
    ...
    grown <- VM.grow initial (25 * (VM.length initial))
    return grown
...
fetch state mode n = case mode of
  ...
  where access n = {- traceShow ((show mode)++"<"++(show n)) $ -} VM.read (memory state) n
...
write state mode n value = do
    addr <- fetch state immediate n
    case mode of
      0 -> go addr
      2 -> go $ (base state) + addr
      _ -> error ("HCF(write mode="++show mode++")"++show (pc state))
  where go n = {- traceShow ((show mode)++">"++(show n)++"/"++(show value)) $ -} VM.write (memory state) n value
        immediate = 1
...
step state = do
    ...
    case {- traceShow ("x="++(show y)++",y="++(show y)++(show $ cycles state)++":"++(show op)) -} op of
